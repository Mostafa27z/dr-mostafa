        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
                <div class="bg-gradient-to-r from-purple-500 to-indigo-600 px-6 py-4 flex justify-between items-center">
                    ...
                    <a href="{{ route('courses.create') }}" class="bg-white text-purple-600 px-4 py-2 rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden course-card">
                    <div class="h-48 bg-gradient-to-r from-blue-400 to-blue-600 relative">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
                            ...
                            <p class="text-blue-200">{{ $course->lessons_count }} دروس</p>
                        </div>
                    </div>
                    <div class="p-5">
                        <p class="text-gray-600 mb-4 line-clamp-2">{{ $course->description ?? 'لا يوجد وصف للدورة' }}</p>
                        <div class="flex justify-between items-center mb-3">
                            <span class="text-2xl font-bold text-purple-600">{{ $course->price }} جنيه</span>
                            <span class="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">
                                {{ $course->enrollments_count }} طالب
                            </span>
                        </div>
                        <div class="flex space-x-2 space-x-reverse">
                            <a href="{{ route('courses.show', $course) }}" class="flex-1 bg-blue-500 text-white py-2 px-4 rounded-xl hover:bg-blue-600 transition-colors text-center">
                                <i class="fas fa-eye ml-2"></i>
                                عرض
                            </a>
                            <a href="{{ route('courses.edit', $course) }}" class="flex-1 bg-gray-500 text-white py-2 px-4 rounded-xl hover:bg-gray-600 transition-colors text-center">
                            ...
                            <form action="{{ route('courses.destroy', $course) }}" method="POST" class="flex-1">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full bg-red-500 text-white py-2 px-4 rounded-xl hover:bg-red-600 transition-colors" onclick="return confirm('هل أنت متأكد من حذف هذه الدورة؟')">
            ...
            <div class="text-center py-12">
                ...
                <a href="{{ route('courses.create') }}" class="text-purple-600 hover:text-purple-800 mt-2 inline-block">
                    إضافة أول دورة
                </a>
            </div>
            @endif
