<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            ...
            إضافة سؤال جديد لامتحان: {{ $exam->title }}
        </h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6">
                <form action="{{ route('exam_questions.store', $exam->id) }}" method="POST" class="space-y-4">
                    @csrf

                    <div>
                        <label for="question_text" class="block text-gray-700 dark:text-gray-300 mb-2 font-medium">نص السؤال</label>
                        <textarea name="question_text" id="question_text" class="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>{{ old('question_text') }}</textarea>
                    </div>
