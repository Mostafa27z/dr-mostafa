...
<div class="bg-white dark:bg-slate-950 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-2xl overflow-hidden">
    ...
    <div class="p-10 text-right">
        <form action="{{ route('exam_questions.store', $exam->id) }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div>
                ...
                <div class="relative group/input">
                    <textarea name="question_text" id="question_text" class="w-full px-8 py-5 bg-gray-50 dark:bg-slate-900/50 border-2 border-transparent focus:border-primary-500 dark:focus:border-primary-500 rounded-[2rem] text-slate-800 dark:text-white font-black transition-all outline-none resize-none" required>{{ old('question_text') }}</textarea>
                    <i class="fas fa-heading absolute left-6 top-6 text-gray-300 dark:text-slate-700 group-focus-within/input:text-primary-500 transition-colors"></i>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative group/input">
                    <select name="type" id="type" class="w-full px-8 py-5 bg-gray-50 dark:bg-slate-900/50 border-2 border-transparent focus:border-primary-500 dark:focus:border-primary-500 rounded-[2rem] text-slate-800 dark:text-white font-black transition-all outline-none appearance-none cursor-pointer" required>
                        <option value="mcq">اختيار من متعدد</option>
                        <option value="essay">مقالي</option>
                    </select>
                    <i class="fas fa-list absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 dark:text-slate-700 group-focus-within/input:text-primary-500 transition-colors pointer-events-none"></i>
                </div>
            </div>
